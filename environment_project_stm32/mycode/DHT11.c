...
void DHT11(void)
{
    ...
    {
//        printf("数据校验成功!\r\n");
//		sprintf(dht11_buf,"数据校验成功!\r\n");
//		HAL_UART_Transmit(&huart1, (uint8_t*)dht11_buf, strlen(dht11_buf), 100);
        UsartPrintf(USART_DEBUG, "数据校验成功!\r\n");
    }
    else
    {
//        printf("DHT11没有应答，请检查传感器!\r\n");
		sprintf(dht11_buf,"DHT11没有应答，请检查传感器!\r\n");
		HAL_UART_Transmit(&huart1, (uint8_t*)dht11_buf, strlen(dht11_buf), 100);
...
//            printf("当前湿度：%d.%d%%RH当前温度：%d.%d°C--",data[0],data[1],data[2],data[3]);
			sprintf(dht11_buf,"当前湿度：%d.%d%%RH当前温度：%d.%d°C--",data[0],data[1],data[2],data[3]);
			HAL_UART_Transmit(&huart1, (uint8_t*)dht11_buf, strlen(dht11_buf), 100);
			humi=data[0];humi_dec=data[1];temp=data[2];temp_dec=data[3];
